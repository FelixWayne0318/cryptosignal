...
def score_trend(
    ...
    # 配置项（缺省给出合理默认）——与仓库 params.json 对齐
    ...
    # 2) 斜率强度（用近 slope_lookback 的收盘做简单线性回归，斜率 / ATR）
    ...
    # 3) R² 置信度 —— 兼容：若上游给的是列表/元组，则收敛成标量
    r2_val = _scalar(r2, 0.0)
    # 把 r2 ~ [0.45, 0.70+] 映射到 [0, 40] 的加分区间
    boost = 40.0 * max(0.0, min(1.0, (r2_val - 0.45) / 0.25))
    # 趋势方向一致时，奖励更多；相反时减弱奖励
