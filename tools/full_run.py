sudo -u linuxuser -H bash -lc 'mkdir -p /opt/ats-quant/tools && cat > /opt/ats-quant/tools/full_run.py << "PY"
# coding: utf-8
...
# Try ats_core; otherwise local minimal implementation
_USING_CORE = False
try:
...
if not _USING_CORE:
    ...
    # ---------- Overlay candidates from params.yml ----------
    ...
    def telegram_send(text: str,
        ...
        if not chat_id:
            raise RuntimeError("TELEGRAM_CHAT_ID(_WATCH/_PRIME) not set")
    ...
    # ---------- Simple renderer (ASCII only) ----------
    def render_watch(res: Dict[str, Any]) -> str:
        s = res.get("symbol", "?")
        prime = bool(res.get("prime", False))
        header = f"[PRIME] {s}" if prime else f"[WATCH] {s}"
        lines = [header]

        def _fmt(k: str, label: Optional[str] = None, suffix: str = ""):
            v = res.get(k)
            if v is None:
                return
    ...
    # ---------- Minimal analysis using Binance 24h ticker ----------
...
def maybe_tag(text: str, is_prime: bool, add_tags: bool) -> str:
    if not add_tags:
        return text
    prefix = "[PRIME]" if is_prime else "[WATCH]"
    return f"{prefix}\n{text}"
    ...
    for s in symbols:
        try:
            ...
            if not isinstance(res, dict):
                raise RuntimeError("analyze_symbol returns non-dict")
            res["symbol"] = s
