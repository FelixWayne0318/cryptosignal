...
## 🎓 实战案例：v7.4.2四步系统硬编码清理（高级配置化）

### 案例背景

**问题来源**: SYSTEM_HEALTH_CHECK_REPORT_2025-11-18.md识别的P1硬编码问题

**修复范围**: v7.4.2四步分层决策系统中的3个P1硬编码问题
...
def calculate_flow_momentum(
    ...
    """
    Args:
        flow_weak_threshold: Flow弱阈值（v7.4.2新增，默认1.0）
        base_min_value: base最小值（v7.4.2新增，默认10.0）
    """
...
v7.4.2首次创建`SESSION_STATE.md`，记录：
...
详细修复文档：
- `docs/v7.4.2_HARDCODE_CLEANUP.md` - 完整变更文档（问题描述、修复方案、验证结果）
...
```
commit 892a170
refactor(v7.4.2): 四步系统硬编码清理 - 配置化改造

commit 52aca9a
docs: 添加SESSION_STATE.md记录v7.4.2硬编码清理session状态
```
...
**实战案例**: `ats_core/decision/step1_direction.py` Lines 65-98 (v7.4.2)
...
**实战案例**: `ats_core/decision/step2_timing.py` Lines 67-112 (v7.4.2)
...
**实战案例**: `ats_core/decision/step3_risk.py` Lines 325-350 (v7.4.2)
...
**文档参考**:
- `docs/v7.4.2_HARDCODE_CLEANUP.md` - 完整修复文档
- `SESSION_STATE.md` - Session状态记录示例
